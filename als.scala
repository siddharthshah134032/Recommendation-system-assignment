...
for( i <- 1 to 10){
	...
	val s1 = ratItemVec.map(x => (x._2._2._1,x._2._1 * x._2._2._2.toDouble )).reduceByKey(_+_) 
	val joinres = userbyItemMat.join(s1) 
...
val ratUserVec = myuserMatrix.join(ratingByUser.value)
	...
	val s2 = ratUserVec.map(x => (x._2._2._1,x._2._1 * x._2._2._2.toDouble )).reduceByKey(_+_) 
	val joinres1 = itembyUserMat.join(s2) 
